...
def compute_eer(target_scores, nontarget_scores, file_name=None):
    """ Returns equal error rate (EER) and the corresponding threshold. """
    frr, far, thresholds = compute_det_curve(target_scores, nontarget_scores)
    # print(frr.shape, far.shape, thresholds.shape)
    if file_name:
        # print(thresholds.T.shape, frr.T.shape, far.T.shape)
        tmp = np.concatenate((np.reshape(thresholds, (len(thresholds), 1)),
                              np.reshape(frr, (len(frr), 1)),
                              np.reshape(far, (len(far), 1))), axis=1)
        if not os.path.exists(det_file):
            os.mkdir(det_file)
        np.savetxt(os.path.join(det_file, file_name), tmp)
